...
GEO_IN VS_LineV2(in VS_INPUT input)      // VS for Curves
{
  ...
  //	input.Position.x *= shift; //äëÿ ìàñøòàáèðîâàíèÿ
...
//  int TempPos = input.Position.x; TempPos &= 0x0000ffff; float x = TempPos;  	x /=32767;
 // TempPos = input.Position.y; TempPos >>= 16; TempPos &= 0x0000ffff; float y = TempPos;  	y /=32767;
	...
	input.Position.x += shift*1; // äëÿ îäèíî÷íûõ êðèâûõ 
	output.num=input.num;

   // output.Position = float4(input.Position,0.0f,1.0f);
 	output.Position = mul(float4(input.Position, 0.0f, 1.0f), WorldViewProjection);

	uint colorTemp = input.Color;   colorTemp &= 0x000000ff;  float b = (float)colorTemp/255;
	colorTemp = input.Color;   colorTemp >>= 8; colorTemp &= 0x000000ff;  float g = (float)colorTemp/255;
	colorTemp = input.Color;   colorTemp >>= 16; colorTemp &= 0x000000ff;  float r = (float)colorTemp/255;
...
GEO_IN_GRID VS_Line_GRID(in VS_INPUT input)    // VS for GRID
{
	...
	output.width = mul (input.num,WorldViewProjection);      //!!!!!!!size use for width
	//output.Position = float4(input.Position,0.0f,1.0f);  
  	output.Position = mul(float4(input.Position, 0.0f, 1.0f), WorldViewProjection); 
	uint colorTemp = input.Color;   colorTemp &= 0x000000ff;  float b = (float)colorTemp/255;
	colorTemp = input.Color;   colorTemp >>= 8; colorTemp &= 0x000000ff;  float g = (float)colorTemp/255;
...
void GS_Curve(line GEO_IN points[2], inout TriangleStream<GEO_OUT> output)    // GS for CURVES
{
	...
	//float width1 = width;
	//float width1 = mul (width,WorldViewProjection);

	float width1 = mul (points[0].num,WorldViewProjection);
  ...
  //	float width = 0.1;
	...
	float3 dir_offset = dir * ratio * width1;          // äëÿ ïåðåêðûòèÿ êîíöîâ ëèíèé
	float3 normal_scaled = normal * ratio * width1;

	float3 p0_ex = p0 - dir_offset;
	float3 p1_ex = p1 + dir_offset;

	v[0].Position = float4(p0_ex - normal_scaled, 1);//* w0; //  v[0].Position.x += v[0].Position.z*shift;
	//v[0].Position = mul (v[0].Position,WorldViewProjection);
	v[0].Color = points[0].Color;

	v[1].Position = float4(p0_ex + normal_scaled, 1);//* w0; //  v[1].Position.x += v[1].Position.z*shift;
	//v[1].Position = mul (v[1].Position,WorldViewProjection);
	v[1].Color = points[0].Color;

	v[2].Position = float4(p1_ex + normal_scaled, 1);//* w1; //  v[2].Position.x += v[2].Position.z*shift;
//	v[2].Position = mul (v[2].Position,WorldViewProjection);
	v[2].Color = points[0].Color;

	v[3].Position = float4(p1_ex - normal_scaled, 1);//* w1; //  v[3].Position.x += v[3].Position.z*shift;
	//v[3].Position = mul (v[3].Position,WorldViewProjection);
...
void GS_GRID(line GEO_IN_GRID points[2], inout TriangleStream<GEO_OUT> output)       // GS for GRID
{
	...
	float tempDif=0.0000001f;

	if(abs(tempXp0-tempXp1)<tempDif)p0.x=tempXp0-tempWidth;
	if(abs(tempYp0-tempYp1)<tempDif)p0.y=tempYp0-tempWidth;
	v[0].Position = p0;
	v[0].Color = points[0].Color;
	...
	if(abs(tempXp0-tempXp1)<tempDif)p1.x=tempXp1+tempWidth;
	if(abs(tempYp0-tempYp1)<tempDif)p1.y=tempYp1+tempWidth;
	v[2].Position = p1;
	v[2].Color = points[0].Color;
